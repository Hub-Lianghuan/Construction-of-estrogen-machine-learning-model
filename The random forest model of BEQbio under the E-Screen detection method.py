...
# 1. 加载数据
data = pd.read_excel(r"C:\Users\LiangHuan\Desktop\原始数据合并版本1987.xls", sheet_name='BEQbio(E-Screen）提取s')
...
# 4. 创建随机森林模型并设置已知的超参数
model = RandomForestRegressor(n_estimators=1000, max_depth=10, min_samples_split=2, min_samples_leaf=1, max_features='sqrt', random_state=42)
...
# 10. 保存模型和标准化器
dump(multioutput_model, r'C:\Users\LiangHuan\Desktop\全部\BEQ-bio(E-SCREEN)\best_model.joblib')  # 保存训练好的模型
dump(scaler_X, r'C:\Users\LiangHuan\Desktop\全部\BEQ-bio(E-SCREEN)\scaler_X.joblib')  # 保存输入特征的标准化器
dump(scaler_y, r'C:\Users\LiangHuan\Desktop\全部\BEQ-bio(E-SCREEN)\scaler_y.joblib')  # 保存目标变量的标准化器
...
for i in range(y_scaled.shape[1]):
    ...
    plt.scatter(y_train_pred_scaled[:, i], y_train_scaled[:, i], color='blue', label='Train Points_
